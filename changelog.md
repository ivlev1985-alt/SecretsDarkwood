...
## [Магазин 2.0] — предметная концепция (старый магазин характеристик/заклинаний удалён)
- items_config.json (JSONC с комментариями): 8 слотов, 4 тира (серый/синий/фиолетовый/оранжевый, статов 1/2/3/4), диапазоны 9 характеристик, 17 шаблонов, цены/обновление
- Шляпа→урон, одежда→HP, сапоги→скорость, перчатки→подбор, браслеты/амулеты полностью рандом; посохи без статов — дают заклинание на выбор (редкость = стартовый уровень 1–4, остаётся навсегда)
- Магазин: 6 офферов (слоты не повторяются), обновление раз в час + кнопка за рекламу (кулдаун 5 мин), скролл колесом; покупка в занятый слот запрещена
- Инвентарь: 8 слотов 4×2, выбор → характеристики цветом редкости + Продать (50%); HUD-инвентарь показывает снаряжение + активные спеллы
- Питомцы (4, радиус 120–240 × тир): летают за одним дропом (монеты/XP) и несут хозяину; монстры их не бьют
- Снаряжение применяется на старте забега; shop_config.json удалён; локализация +37 ключей (6/6 языков, всего 152)

## [Магазин 2.1] — фиксы по скриншоту
- Причина мёртвых кнопок: в ShopMenu использовался ShopSystem без import — отрисовка падала после заголовка первой карточки (прямоугольники кликов/закрытия не создавались). Import добавлен
- Раскладка: таймер + «Обновить» переехали вниз (правая грань кнопки под правой гранью карточек, стиль primary); список снова 3 карточки + скролл
- Инвентарь: кнопка «Продать» сразу под названием, характеристики ниже — наложений нет
- Проверено моками: отрисовка обеих вкладок без падений, покупка в 2 тапа, слот занимается, продажа +50%, выбор слота

## [Инвентарь 2.0 + питомец-магнит]
- Инвентарь: сетка 2×4, колонки строго 50..224 / 232..406 (симметрия); иконка на всю ячейку (заливка цвета редкости + название); inv_empty убран
- Детализация: название + характеристики столбиком по левому краю (x=50); «Продать» 50..406 сразу над «Закрыть»
- Питомец-магнит: монеты/XP в радиусе притягиваются к питомцу (как к персонажу), подбор/доставка по одному — как раньше
- Апгрейды питомца в levelup (только с питомцем): +40 дальности и +20 притяжения за уровень, макс 5; дефолт притяжения 60 из items_config.pet
- Локализация +4 ключа (156/156 все языки)

## [Аудит статов + сетка 4×2]
- АУДИТ: 8/9 характеристик работают; найден и починен крит — CombatSystem копировал шанс из базового конфига и игнорировал снаряжение/пассивки (теперь sync с игроком каждый кадр; проверено: 5% не критует на 0.999, 10% критует ×2)
- Работают: урон/HP/скорость/скорость атаки/подбор/реген/опыт (цепочка gear→base→player→бой, 10/10 тестов), радиус питомца, заклинание посоха
- Инвентарь: сетка 4×2 квадратных слотов 83px (50/141/232/323 × 245/336, правая грань 406); иконки-заглушки на ячейку сохранены

## [Особый питомец]
- Ворон убран из магазина (special), падает только из легендарного сундука 10% (легендарный, радиус 480, цена 3000)
- Особый питомец таскает ещё и аптечки (флаг fetch_potions; обычный их игнорирует, проверено)
- Модалка замены с паузой мира: слева текущий, справа выпавший (иконка + характеристики), кнопки Закрыть/Поменять; старый автовыкупается за 50%
- TEST (убрать позже): TEST_LEGENDARY_CHEST и TEST_PET_DROP=true — стартовый легендарный сундук с гарант. питомцем
- Локализация +5 ключей (161/161 все языки)

## [Мелочь]
- Круг-подсказка джойстика теперь только на тачскринах (isTouch по ontouchstart/maxTouchPoints + автоопределение при первом таче); на ПК его нет, активный джойстик рисуется везде как раньше

## [Мелочь 2]
- Первый спавн монстров на 10-й секунде (было 15)
- Смерч и Огненная стена не кастуются без монстров (короткий ретрай 0.25с вместо пустого каста)

## [Тапы, питомец, спека]
- Фикс фантомных тапов: touchend теперь с preventDefault — один тап больше не закрывает попап и тут же не жмёт меню под ним (причина авто-открытия магазина)
- Питомец: синусоида полёта (боковое колебание 30px/s, проверено отклонение 33px)
- docs/assets_spec.md: полная спека графики из конфигов (пути, кадры, ряды, fps) для художника

## [Мелочи 2]
- Инвентарь: названия предметов мелким шрифтом в 1–2 строки (лимит 12 символов/строка ≈ 79px из 83) — влезают полностью
- Пауза на Пробел (как Esc/P); открытый инвентарь замораживает мир (авто-пауза без меню паузы)
